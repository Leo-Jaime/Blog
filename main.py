...
# TODO: Crie uma tabela de usuários para todos os seus usuários registrados.
...
# TODO: Use Werkzeug para fazer hash da senha do usuário ao criar um novo usuário.
@app.route('/register', methods=["GET", "POST"])
def register():
    form = RegisterForm()
    if form.validate_on_submit():
        if db.session.execute(db.select(User).where(User.email == form.email.data)).scalar():
            flash(message="Esse Email já esta registrado")
            return redirect(url_for("register"))
        else:
            novo_usuario = User(
                name= form.name.data,
                email= form.email.data,
                password= generate_password_hash(password=form.password.data, salt_length=8, method="pbkdf2")
            )
...
# TODO: Recuperar um usuário do banco de dados com base em seu email.
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if not user:
            flash(message="Email não cadastrado")
...
@app.route('/')
def get_all_posts():
    #caso o usuario não esteja autenticado, ele vai ser redirecionado para o login
...
# TODO: Permitir que usuários logados comentem nas postagens
...
def show_post(post_id):
    ...
    if comment_form.validate_on_submit():
        if not current_user.is_authenticated:
            flash(message="Você precisa realizar o login!!!")
            return redirect(url_for("login"))
        else:

            novo_comentario = Comment(
                texto= comment_form.texto.data,
...
# TODO: Use um decorador para que apenas um usuário administrador possa criar uma nova postagem
...
# TODO: Use um decorador para que apenas um usuário administrador possa editar uma postagem
...
# TODO: Use um decorador para que apenas um usuário administrador possa excluir uma postagem
